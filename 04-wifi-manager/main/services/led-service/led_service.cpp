...
LedService::LedService(TimeService& timeService,GPIOManager& gpioManager,HTTPClientService& httpClientService):_timeService(timeService),_gpioManager(gpioManager),_httpClientService(httpClientService){}
...
void LedService::init(){
    for(;;){
        if(_button->click()){
            ...
            cJSON_AddStringToObject(json.get(),"state",_led->isON() ?"ON":"OFF");
            cJSON_AddNumberToObject(json.get(),"timestamp",_timeService.getNow());
            cJSON_AddNumberToObject(jsonState.get(),"pin",_led->getPin());
            cJSON_AddBoolToObject(jsonState.get(),"state",_led->isON());
